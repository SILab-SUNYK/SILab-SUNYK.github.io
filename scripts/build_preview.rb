...
FileUtils.rm_rf(OUT)
FileUtils.mkdir_p(File.join(OUT, "assets/css"))
FileUtils.cp(File.join(ROOT, "assets/css/style.css"), File.join(OUT, "assets/css/style.css"))
...
def wrap(title, body)
  <<~HTML
  <!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>#{title} · SIL</title><link rel="stylesheet" href="/assets/css/style.css"></head><body>
  #{header}<main>#{body}</main>
  <footer class="site-footer wrap"><div><b>SIL</b><span>Spatial Intelligence Lab</span></div><p>Machines that understand space.</p><div><a href="mailto:francoisbernar.rameau@stonybrook.edu">Contact ↗</a><small>SUNY Korea · Stony Brook University</small></div></footer></body></html>
  HTML
end

home = links(strip_front(File.read(File.join(ROOT, "index.html"))))
cards = pubs.first(3).map { |p| %(<a href="#{p['link']}"><span>#{p['type']}<b>#{p['venue']} · #{p['year']}</b></span><h3>#{p['title']}</h3><p>#{p['authors']}</p><i>↗</i></a>) }.join
...
archive = pubs.group_by { |p| p['year'] }.map do |year, list|
  items = list.map { |p| %(<a class="archive-paper" data-type="#{p['type']}" href="#{p['link']}"><span>#{p['type']}<b>#{p['venue']}</b></span><h3>#{p['title']}</h3><p>#{p['authors']}</p><i>↗</i></a>) }.join
